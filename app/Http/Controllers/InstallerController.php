...
class InstallerController extends Controller
{
    ...
    public function saveWizard(Request $request)
    {
        ...
        try {
            ...
            if ($res['results']) {
                ...
                // $this->installer->SetCustomerDetails($data['customer_details']);
                $final = $this->installer->finish($same_directory,$data['customer_details']);
            ...
            return response()->json(['status_code' => 500, 'errors' => 'somethwrong']);
        }
    ...
    public function testDBConnection(Request $request)
    {
        try {
            ...
            DB::connection($request->database_connection)->getPdo();
        } catch (\Exception $e) {
            if ($e instanceof \Illuminate\Validation\ValidationException) {
                $errors = $e->errors();
                return response()->json(['status_code' => 422, 'errors' => $errors, 'message' => 'somerong']);
            }
